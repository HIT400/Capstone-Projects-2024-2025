...
/**
 * Migration script to update the role constraint in the users table
 */
const updateUserRoleConstraint = async () => {
  try {
    console.log("Starting migration: Updating users role constraint");
    ...
    if (checkResult.rows.length > 0) {
      ...
      if (constraintResult.rows.length > 0) {
        ...
        console.log("Migration successful: users_role_check constraint updated");
      } else {
        console.log("Constraint users_role_check not found");
      }
    } else {
      console.log("Migration skipped: users table does not exist");
    }
...
export default updateUserRoleConstraint;
