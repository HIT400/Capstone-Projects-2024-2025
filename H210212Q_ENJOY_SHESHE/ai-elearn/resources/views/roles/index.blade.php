...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-lg font-semibold text-gray-900">Roles</h1>
            </div>
        ...
        <div class="mt-6 overflow-x-auto">
            <table class="w-full border-collapse border border-gray-300">
                ...
                <tr>
                    <th class="px-6 py-3 text-left border border-gray-300">Role Name</th>
                ...
                @foreach ($roles as $role)
                    <tr class="border border-gray-300">
                        <td class="px-6 py-3.5 border border-gray-300">{{ $role->name }}</td>
                        <td class="px-6 py-3.5 border border-gray-300">{{ $role->status }}</td>
                        <td class="px-6 py-3.5 border border-gray-300">
                            <div class="flex space-x-4">
                                <button onclick="openEditModal({{ $role->id }}, '{{ $role->name }}', '{{ $role->status }}')"
    ...
    <div id="addModal" class="fixed inset-0 hidden bg-gray-900 bg-opacity-50 flex justify-center items-center">
        <div class="bg-white p-6 rounded-md shadow-md w-1/3">
            <h2 class="text-lg font-semibold text-gray-900">Add Role</h2>
