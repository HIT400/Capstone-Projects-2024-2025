...
@section('content')
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="sm:flex sm:items-center">
            <div class="sm:flex-auto">
                <h1 class="text-lg font-semibold text-gray-900">Interests</h1>
            </div>
        ...
        <div class="mt-6 overflow-x-auto">
            <table class="w-full border-collapse border border-gray-300">
                ...
                <tr>
                    <th class="px-6 py-3 text-left border border-gray-300">Interest Name</th>
                ...
                @foreach ($interests as $interest)
                    <tr class="border border-gray-300">
                        <td class="px-6 py-3.5 border border-gray-300">{{ $interest->name }}</td>
                        <td class="px-6 py-3.5 border border-gray-300">{{ $interest->status }}</td>
                        <td class="px-6 py-3.5 border border-gray-300">
                            <div class="flex space-x-4">
                                <button onclick="openEditModal({{ $interest->id }}, '{{ $interest->name }}', '{{ $interest->status }}')"
                                        class="text-indigo-600 hover:text-indigo-900">Edit</button> |
                                <form action="{{ route('interest-destroy', $interest->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
    ...
    <div id="addModal" class="fixed inset-0 hidden bg-gray-900 bg-opacity-50 flex justify-center items-center">
        <div class="bg-white p-6 rounded-md shadow-md w-1/3">
            <h2 class="text-lg font-semibold text-gray-900">Add Interest</h2>
            <form action="{{ route('interest-store') }}" method="POST" class="mt-4">
                @csrf

                <label class="block text-sm font-medium text-gray-700">Interest Name</label>
    ...
    <div id="editModal" class="fixed inset-0 hidden bg-gray-900 bg-opacity-50 flex justify-center items-center">
        <div class="bg-white p-6 rounded-md shadow-md w-1/3">
            <h2 class="text-lg font-semibold text-gray-900">Edit Interest</h2>
    ...
    <script>
        ...
        function openEditModal(id, name, status) {
            ...
            document.getElementById('editForm').action = `/interests/${id}`;
        }
